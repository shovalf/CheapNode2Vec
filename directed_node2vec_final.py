...
def get_initial_proj_nodes(G, key):
    ...
    :param G: Our graph
    :param key: Controls number of nodes on the initial projection
...
def our_node2vec(G, dim):
    """
    A function to do node2vec embedding on graph G.
    :param G: Our graph
...
def create_dicts_of_connections(set_proj_nodes, set_no_proj_nodes, neighbors_dict, G):
    ...
    :return: 6 dictionaries, explained above
    """
    dict_node_node_out = {}
    dict_node_node_in = {}
    dict_node_enode = {}
...
def calculate_average_projection_second_order(dict_proj, node, dict_enode_enode, average_two_order_proj):
    """
    ...
    # else, the average projection in 0
    else:
...
def calculate_projection_of_neighbors(proj_nodes, dict_proj):
    """
...
def calculate_projection(proj_nodes_in, proj_nodes_out, dict_proj, dim, alpha1, alpha2, beta_11, beta_12, beta_21, beta_22):
    """
    A final to calculate the final projection of the node by a formula that is in the github page.
    ...
    if len(proj_nodes_in) > 0:
        proj_in, proj_in_in, proj_in_out, k1, k2 = calculate_projection_of_neighbors(
            proj_nodes_in, dict_proj)
    else:
        proj_in, proj_in_in, proj_in_out = np.zeros(dim), np.zeros(dim), np.zeros(dim)
    if len(proj_nodes_out) > 0:
        proj_out, proj_out_in, proj_out_out, k1, k2 = calculate_projection_of_neighbors(
            proj_nodes_out, dict_proj)
    else:
        proj_out, proj_out_in, proj_out_out = np.zeros(dim), np.zeros(dim), np.zeros(dim)
    # the final projection of the node
    final_proj = alpha1*proj_in+alpha2*proj_out - beta_11*proj_in_in - beta_12*proj_in_out - \
                 beta_21*proj_out_in - beta_22*proj_out_out
    return final_proj
...
def one_iteration(params, dict_enode_proj, dict_node_enode, dict_enode_node, dict_node_node_in, dict_node_node_out,
    ...
    if dict_enode_node.get(current_node) is not None:
        ...
        embd_neigh_in = set()
    # the final projection of the node
    final_proj = calculate_projection(embd_neigh_in, embd_neigh_out, dict_enode_proj, dim,
    ...
    for i in range(len(initial_proj_nodes)):
        ...
        if dict_enode_enode_in.get(node) is not None:
            x_1, z_11, z_12, k1, k2 = calculate_projection_of_neighbors(
                dict_enode_enode_in[node], dict_intial_projections)
        else:
        ...
        if dict_enode_enode_out.get(node) is not None:
            x_2, z_21, z_22, k1, k2 = calculate_projection_of_neighbors(
                dict_enode_enode_out[node], dict_intial_projections)
        else:
        ...
        if np.array_equal(a, b) is False:
            nodes.append(node)
            X = np.column_stack((x_1, x_2, z_11, z_12, z_21, z_22, np.ones(10)))
...
def linear_regression(dict_params, nodes, dict_proj):
    ...
    reg.fit(X, Y)
    pre_params = reg.coef_
    score = reg.score(X, Y)
    return pre_params, score
...
if __name__ == "__main__":
    ...
    initial_proj_nodes = get_initial_proj_nodes(G, 0.88)
    ...
    final_dict_enode_proj, set_n_e = final_function(pre_params, dict_projections,
    ...
    print(set_n_e)
    print(len(final_dict_enode_proj))
